...
class Simulation:
    ...
    def mass_balance(self):
        ...
        def soil_stores(surface):
            return ((surface.imp_store*surface.imp_frac) + (surface.a1*surface.store1 + surface.a2*surface.store2 + surface.a3*surface.store3)*(1-surface.imp_frac))*surface.area / 1000.0

        # inflows
        rainfall_volume = sum([self.rainfall.sum() / 1000.0 * surface.area for surface in self.surfaces.values()])
        external_supply = sum([main.supply.sum() for main in self.mains.values()])
        initial_storage = sum([storage.initial_volume for storage in self.storages.values()])
        ...
        storage_leakage = sum([storage.leakage.sum() for storage in self.storages.values()])
        water_usage = sum([demand.source_supplied.sum() for demand in self.demands.values()])
        end_storage_volumes = sum([storage.volume[-1] for storage in self.storages.values()])
        end_soil_stores = sum([soil_stores(surface)[-1] for surface in self.surfaces.values()])
        end_baseflow_stores = sum([surface.baseflow_store[-1] for surface in self.surfaces.values()])
        end_runoff_stores = sum([surface.runoff_store[-1] for surface in self.surfaces.values()])
        outflow_volume = surface_et + storage_et + storage_leakage + water_usage + end_storage_volumes + end_soil_stores + end_baseflow_stores + end_runoff_stores
...
if __name__ == "__main__":
    ...
    silo_data =  pd.read_csv("-27.45_153.00.csv", index_col="YYYY-MM-DD", parse_dates=True)
